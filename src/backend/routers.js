...
export function shop_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("shop-page", data, seoData); 
   }
...
export function shop_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "shop-page";		// The name of the page in the Wix editor to render
                               entry.url = "/shop/" + name ;			// Relative URL of the page
