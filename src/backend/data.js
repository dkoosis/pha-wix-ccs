...
export async function StudioMembershipApplications_afterUpdate(item, context) {
  ...
  console.log(
    `Processing approval for application ${item._id} from ${item.email}`
  );
  ...
  try {
    ...
    if (existingMembers.items.length > 0) {
      ...
      if (existingContacts.items.length > 0) {
        ...
        console.log(
          `Found existing contact: ${contact._id}. Upgrading to member...`
        );
    ...
    await authorization.assignRole(ROLE_IDS.CCS_INVITEE, memberId, {
      suppressAuth: true,
    });
    ...
    console.error(
      `Error processing approval for application ${item._id}:`,
      error
    );
  }
...
export async function MemberProfiles_afterUpdate(item, context) {
  ...
  try {
    ...
    await authorization.removeRole(ROLE_IDS.CCS_INVITEE, memberId, {
      suppressAuth: true,
    });

    console.log("Assigning CCS_Member role...");
    await authorization.assignRole(ROLE_IDS.CCS_MEMBER, memberId, {
      suppressAuth: true,
    });

    console.log(
      `✅ Successfully promoted member ${memberId} to full membership`
    );
  } catch (error) {
