...
.button-container {
display: flex;
flex-direction: row; /* stacked vertically like in the image */
gap: 2rem;
margin-top: 2rem;
align-items: center;
}
...
cursor: not-allowed;
  ...
  _renderTemplate() {
    ...
    template.innerHTML = `
            ...
            <table>
                <thead>
                    <tr>
                        ${CeramicsFiringCalculator.HEADERS.map(
                          (header) => `<th>${header}</th>`
                        ).join("")}
                    </tr>
                </thead>
  ...
  _attachEventListeners() {
    ...
    const submitWorksheetButton = this.shadowRoot.getElementById(
  ...
  handleDelete(event) {
    ...
    if (
      deleteButton &&
      deleteButton.hasAttribute("data-action") &&
  ...
  updateDeleteButtonState() {
    ...
    const deleteButtons = this.dataRows.querySelectorAll(
      '[data-action="delete"]'
    );
  ...
  async processImage(file, row) {
    ...
    try {
      ...
      console.log("optimized", optimizedImage, thumbnail);
  ...
  _createDimensionCell(fieldName, label) {
    ...
    input.setAttribute(
      "aria-label",
      `${label} in inches (${this.MIN_DIMENSION}-${this.MAX_DIMENSION})`
    );
  ...
  _createPhotoUploadCell() {
    ...
    const cameraIcon = this._createSVGIcon(
      CeramicsFiringCalculator.ICONS.CAMERA,
      "camera-icon"
    );
  ...
  calculateRowValues(row) {
    ...
    if (dueDate) {
      ...
      const daysDiff = Math.ceil(
        (selectedDueDate - currentDate) / (1000 * 60 * 60 * 24)
      );
      console.log(
        "  DUE DATE: Value=",
        dueDate,
    ...
    if (priceCell) {
      ...
      console.log(
        "    Updated price cell to:",
        this.USDformatter.format(price)
      );
    } else {
   ...
   */
  updateTotalCost() {
    const total = Array.from(this.dataRows.querySelectorAll("tr")).reduce(
      (sum, row) => {
        const priceCell = row.querySelector('[data-field="price"]');
  ...
  submitWorksheet() {
    try {
      const data = Array.from(this.dataRows.querySelectorAll("tr")).map(
        (row) => {
          // Extract all data using reliable selectors - now simplified since we cleaned up data-field usage
          const firingType = row.querySelector(
            '[data-field="firing-type"]'
          )?.value;
          const height =
            parseInt(row.querySelector('[data-field="height"]')?.value) || 0;
          const width =
            parseInt(row.querySelector('[data-field="width"]')?.value) || 0;
          const length =
            parseInt(row.querySelector('[data-field="length"]')?.value) || 0;
          const quantity =
            parseInt(row.querySelector('[data-field="quantity"]')?.value) || 0;
          const unitCostText = row.querySelector(
            '[data-field="unit-cost"]'
          )?.textContent;
          const unitCost =
            parseFloat(unitCostText?.replace(/[^0-9.-]+/g, "")) || 0;
          const volume =
            parseInt(row.querySelector('[data-field="volume"]')?.textContent) ||
            0;
          ...
          const dueDate =
            row.querySelector('[data-field="due-date"]')?.value || null;
          const specialDirections =
            row.querySelector('[data-field="directions"]')?.value || null;
      ...
      this.dispatchEvent(
        new CustomEvent("submitWorksheet", {
          detail: { data },
          bubbles: true,
        })
      );
  ...
  _validateWorksheetData(data) {
    ...
    data.forEach((item, index) => {
      ...
      if (
        item.height < this.MIN_DIMENSION ||
        item.height > this.MAX_DIMENSION
      ) {
        errors.push(
          `Row ${rowNum}: Height must be ${this.MIN_DIMENSION}-${this.MAX_DIMENSION} inches`
        );
      }
      if (item.width < this.MIN_DIMENSION || item.width > this.MAX_DIMENSION) {
        errors.push(
          `Row ${rowNum}: Width must be ${this.MIN_DIMENSION}-${this.MAX_DIMENSION} inches`
        );
      }
      if (
        item.length < this.MIN_DIMENSION ||
        item.length > this.MAX_DIMENSION
      ) {
        errors.push(
          `Row ${rowNum}: Length must be ${this.MIN_DIMENSION}-${this.MAX_DIMENSION} inches`
        );
      }
      if (item.quantity <= 0 || item.quantity > this.MAX_QUANTITY) {
        errors.push(
          `Row ${rowNum}: Quantity must be 1-${this.MAX_QUANTITY} pieces`
        );
      }
